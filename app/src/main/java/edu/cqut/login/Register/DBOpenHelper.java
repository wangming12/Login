package edu.cqut.login.Register;
...
public class DBOpenHelper extends SQLiteOpenHelper {
    ...
    public void add(String name,String password) {
        sqLiteDatabase.execSQL("INSERT INTO user (name,password) VALUES(?,?)",new Object[]{name,password});
    }

    public void delete(String name,String password) {
        sqLiteDatabase.execSQL("DELETE FROM user WHERE name = AND password ="+name+password);
    }
    ...
    public ArrayList<User> getAllData() {
        ...
        Cursor cursor = sqLiteDatabase.query("user",null,null,null,null,null,"name DESC");
        while (cursor.moveToNext()) {
            String name = cursor.getString(cursor.getColumnIndex("name"));
            String password = cursor.getString(cursor.getColumnIndex("password"));
            arrayList.add(new User(name,password));
        }
        return arrayList;
    }
}
